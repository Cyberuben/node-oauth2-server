...
AuthorizeHandler.prototype.getClient = function(request) {
  var clientId = request.body.clientId || request.query.clientId;
  ...
  var redirectUri = request.body.redirectUri || request.query.redirectUri;

  if (redirectUri && !is.uri(redirectUri)) {
    throw new InvalidRequestError('redirectUri_invalid');
  }
  return promisify(this.model.getClient, 2)(clientId, null)
    .then(function(client) {
      ...
      if (!_.includes(client.grants, 'authorization_code')) {
        throw new UnauthorizedClientError('authorizationCodeGrant_forbidden');
      }
      ...
      if (redirectUri && !_.includes(client.redirectUris, redirectUri)) {
        throw new InvalidClientError('redirectUri_forbidden');
      }
      return client;
    });
};
...
AuthorizeHandler.prototype.getScope = function(request) {
  ...
  if (!is.nqschar(scope)) {
    throw new InvalidScopeError('scope_invalid`');
  }
...
AuthorizeHandler.prototype.getState = function(request) {
  ...
  if (!this.allowEmptyState && !state) {
    throw new InvalidRequestError('state_missing');
  }

  if (!is.vschar(state)) {
    throw new InvalidRequestError('state_invalid');
  }
...
AuthorizeHandler.prototype.getRedirectUri = function(request, client) {
  return request.body.redirectUri || request.query.redirectUri || client.redirectUris[0];
};
...
AuthorizeHandler.prototype.getResponseType = function(request) {
  var responseType = request.body.responseType || request.query.responseType;

  if (!responseType) {
    throw new InvalidRequestError('responseType_missing');
  }

  if (!_.has(responseTypes, responseType)) {
    throw new UnsupportedResponseTypeError('responseType_invalid');
  }
