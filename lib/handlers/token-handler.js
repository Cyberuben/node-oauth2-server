...
TokenHandler.prototype.getClient = function(request, response) {
  ...
  if ((grantType == "password" && this.trustedClientIds.indexOf(request.body.clientId) == -1) && this.isClientAuthenticationRequired(grantType) && !request.body.clientSecret) {
    throw new InvalidRequestError('clientSecret_empty');
  }

  if (!is.vschar(request.body.clientId)) {
    throw new InvalidRequestError('clientId_invalid');
  }

  if (!is.vschar(request.body.clientSecret)) {
    throw new InvalidRequestError('clientSecret_invalid');
  }

  return promisify(this.model.getClient, 2)(request.body.clientId, request.body.clientSecret)
    .then(function(client) {
      if (!client) {
