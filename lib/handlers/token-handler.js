...
TokenHandler.prototype.getClientCredentials = function(request) {
  ...
  if (request.body.clientId && request.body.clientSecret) {
    return { clientId: request.body.clientId, clientSecret: request.body.clientSecret };
  }

  if (!this.isClientAuthenticationRequired(grantType) || (grantType == "password" && this.trustedClientIds.indexOf(request.body.clientId) != -1)) {
