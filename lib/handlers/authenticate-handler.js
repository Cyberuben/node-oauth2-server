...
AuthenticateHandler.prototype.getTokenFromRequest = function(request) {
  ...
  //var queryToken = request.query.accessToken;
  //var bodyToken = request.body.accessToken;
  ...
  throw new UnauthorizedRequestError('authHeader_missing');
};
...
AuthenticateHandler.prototype.getTokenFromRequestHeader = function(request) {
  ...
  if (!matches) {
    throw new InvalidRequestError('authHeader_invalid');
  }
...
AuthenticateHandler.prototype.getAccessToken = function(token) {
  return promisify(this.model.getAccessToken, 1)(token)
    .then(function(accessToken) {
      if (!accessToken) {
        throw new InvalidTokenError('accessToken_invalid');
      }
...
AuthenticateHandler.prototype.validateAccessToken = function(accessToken) {
  ...
  if (accessToken.accessTokenExpiresAt < new Date()) {
    throw new InvalidTokenError('accessToken_expired');
  }
...
AuthenticateHandler.prototype.verifyScope = function(accessToken) {
  return promisify(this.model.verifyScope, 2)(accessToken, this.scope)
    .then(function(scope) {
      if (!scope) {
        throw new InsufficientScopeError('scope_invalid');
      }
