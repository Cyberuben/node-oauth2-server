...
PasswordGrantType.prototype.getUser = function(request) {
  if (!request.body.email) {
    throw new InvalidRequestError('email_missing');
  }

  if (!request.body.password) {
    throw new InvalidRequestError('password_missing');
  }

  if (!is.uchar(request.body.email)) {
    throw new InvalidRequestError('email_invalid');
  }

  if (!is.uchar(request.body.password)) {
    throw new InvalidRequestError('password_invalid');
  }

  return promisify(this.model.getUser, 2)(request.body.email, request.body.password)
    .then(function(user) {
      if (!user) {
        throw new InvalidGrantError('login_invalid');
      }
